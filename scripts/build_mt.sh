...
BUILD_DIR=../build/mt
CIRCUIT_PATH=../circuits/mt.circom
CIRCUIT_NAME=mt
...
start=`date +%s`
node "$BUILD_DIR"/"$CIRCUIT_NAME"_js/generate_witness.js "$BUILD_DIR"/"$CIRCUIT_NAME"_js/"$CIRCUIT_NAME".wasm input-mt.json "$BUILD_DIR"/witness.wtns
end=`date +%s`
echo "DONE ($((end-start))s)"
